...
from coding_agent.permissions import Decision, PermissionChecker, PermissionMode
from coding_agent.hooks import HookContext, HookEngine, ToolRejectedError
from coding_agent.hooks.engine import HookNotification
from coding_agent.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from coding_agent.tools import ToolRegistry
from coding_agent.tools.base import MAX_OUTPUT_CHARS, StreamEnd, StreamEvent, TextDelta, ThinkingComplete, ThinkingDelta, ToolCallComplete, ToolCallDelta, ToolCallStart, ToolResult
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".coding_agent" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".coding_agent" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from coding_agent.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from coding_agent.context.manager import SINGLE_RESULT_CHAR_LIMIT, make_persisted_preview, persist_tool_result
