...
from coding_agent.serialization import build_anthropic_messages, build_chat_completion_messages, build_openai_input
from coding_agent.tools.base import StreamEnd, StreamEvent, TextDelta, ThinkingComplete, ThinkingDelta, ToolCallComplete, ToolCallDelta, ToolCallStart
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .coding_agent/config.yaml or via ANTHROPIC_API_KEY env var."
            )
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .coding_agent/config.yaml or via OPENAI_API_KEY env var."
            )
...
class OpenAICompatClient(LLMClient):
    ...
    async def stream(
        ...
        # 兼容不同 provider 的 usage chunk 布局：
        #  - OpenAI / DeepSeek 等把 usage 放在一个独立的、不带 choices 的末尾 chunk；
        #  - 智谱 GLM 等把 usage 合并到最后一个“带 choices”的 chunk。
        # 因此我们在每个 chunk 上都捕获 usage，并在流结束时统一 yield StreamEnd。
        pending_usage = None
        stream_end_yielded = False

        def _emit_usage(usage):
            """从 openai usage 对象构造并 yield 一个 StreamEnd。"""
        ...
        try:
            ...
            async for chunk in response:
                if chunk.usage is not None:
                    # 任意 chunk 携带 usage 都记录下来（智谱 GLM 会在最后一个
                    # 带 choices 的 chunk 里返回 usage，而非独立的空 chunk）。
                    pending_usage = chunk.usage

                if not chunk.choices:
                    # 独立的末尾 usage chunk（OpenAI / DeepSeek 风格）。
                    if pending_usage is not None and not stream_end_yielded:
                        yield _emit_usage(pending_usage)
                        stream_end_yielded = True
                        pending_usage = None
                    continue
            ...
            # 流自然结束：若 usage 合并在最后一个带 choices 的 chunk（智谱 GLM
            # 风格）且尚未 yield，则在此补发 StreamEnd。
            if pending_usage is not None and not stream_end_yielded:
                yield _emit_usage(pending_usage)
                stream_end_yielded = True
                pending_usage = None
