...
def main() -> None:
    ...
    from coding_agent.app import CodingAgent
    from coding_agent.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    from coding_agent.agent import Agent, CompactNotification, ErrorEvent, LoopComplete, PermissionRequest, PermissionResponse, RetryEvent, StreamText, ThinkingText, ToolResultEvent, ToolUseEvent, TurnComplete, UsageEvent
    ...
    from coding_agent.memory.instructions import load_instructions
    from coding_agent.permissions import DangerousCommandDetector, PathSandbox, PermissionChecker, RuleEngine
    ...
    registry = create_default_registry(project_root=work_dir)
