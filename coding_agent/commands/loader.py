"""从 .coding_agent/commands/ 目录加载自定义 Markdown 命令（对齐 Go 版 loader.go）。
...
from coding_agent.commands.registry import Command, CommandContext, CommandType
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.coding_agent/commands/
      2. <work_dir>/.coding_agent/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".coding_agent" / "commands"))
    dirs.append(str(Path(work_dir) / ".coding_agent" / "commands"))
