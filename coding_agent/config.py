...
from coding_agent.validator import ConfigError, DEFAULT_CONTEXT_WINDOW, VALID_PERMISSION_MODES, VALID_PROTOCOLS, VALID_TEAMMATE_MODES, lookup_model_context_window, validate_config_structure
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".coding_agent" / "config.yaml",
        cwd / ".coding_agent" / "config.yaml",
        cwd / ".coding_agent" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .coding_agent/config.yaml "
            "in project or ~/.coding_agent/config.yaml"
        )
    return merged
