...
async def test_multi_step_autonomous():
    ...
    test_file = "/tmp/coding_agent_test_hello.txt"
    ...
    client = MockLLMClient([
        ...
        [
            ...
            ToolCallComplete("t1", "WriteFile", {"file_path": "/tmp/coding_agent_test_hello.txt", "content": "Hello World"}),
            StreamEnd("end_turn", input_tokens=10, output_tokens=20),
        ],
        # 第 2 轮：ReadFile 进行验证
        [
            TextDelta("Verifying content."),
            ToolCallComplete("t2", "ReadFile", {"file_path": "/tmp/coding_agent_test_hello.txt"}),
