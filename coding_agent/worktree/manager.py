...
from coding_agent.worktree.changes import CleanupResult, Changes, count_worktree_changes, has_worktree_changes
from coding_agent.worktree.models import Worktree, WorktreeSession
from coding_agent.worktree.session import load_worktree_session, save_worktree_session
from coding_agent.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".coding_agent" / "worktrees"
        )
        self._coding_agent_dir = Path(repo_root) / ".coding_agent"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._coding_agent_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._coding_agent_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._coding_agent_dir)
        if session is None:
            return None
