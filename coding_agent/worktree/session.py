...
def _session_path(coding_agent_dir: Path) -> Path:
    return coding_agent_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(coding_agent_dir)
    if session is None:
...
def load_worktree_session(coding_agent_dir: Path) -> WorktreeSession | None:
    path = _session_path(coding_agent_dir)
    if not path.exists():
        return None
    try:
