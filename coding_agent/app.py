...
from coding_agent.client import AuthenticationError, LLMClient, LLMError, create_client, resolve_context_window
from coding_agent.commands import CommandContext, CommandRegistry, complete, parse_command
from coding_agent.commands.completion import CompletionPopup
...
from coding_agent.hooks import HookContext, HookEngine, load_hooks
from coding_agent.conversation import ConversationManager, Message
from coding_agent.mcp import ConnectResult, MCPManager
from coding_agent.memory import MemoryManager, Session, SessionManager, find_relevant_memories, generate_session_summary, load_instructions, make_compact_boundary, render_reminder
...
from coding_agent.agents.trace import TraceManager
from coding_agent.agents.notification import inject_task_notifications
from coding_agent.commands.handlers.tasks import create_tasks_command
from coding_agent.skills.executor import SkillExecutor
from coding_agent.skills.loader import SkillLoader
from coding_agent.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from coding_agent.cache import FileCache
from coding_agent.tools import ToolRegistry, create_default_registry
from coding_agent.tools.agent_tool import AgentTool
from coding_agent.tools.ask_user import AskUserEvent, AskUserTool
from coding_agent.tools.impl.tool_search import ToolSearchTool
from coding_agent.tools.load_skill import LoadSkill
from coding_agent.worktree.cleanup import start_stale_cleanup_task
from coding_agent.worktree.manager import WorktreeManager
from coding_agent.commands.handlers.worktree import create_worktree_command
from coding_agent.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".coding_agent", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".coding_agent" / "history"
...
class CodingAgent(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_CODING_AGENT_THEME)
        self.theme = "coding_agent"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        from coding_agent.filehistory import FileHistory
        ...
        for tool in self.registry.list_tools():
            ...
            if hasattr(tool, "_project_root") and tool._project_root is None:
                tool._project_root = work_dir
        ...
        # --- Worktree 系统初始化 ---
        from coding_agent.config import WorktreeConfig
        ...
        from coding_agent.tools.enter_worktree import EnterWorktreeTool
        from coding_agent.tools.exit_worktree import ExitWorktreeTool
        ...
        from coding_agent.tools.team_create import TeamCreateTool
        from coding_agent.tools.team_delete import TeamDeleteTool
        ...
        from coding_agent.commands.handlers.trace import create_trace_command
        ...
        from coding_agent.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _show_plan_approval(self) -> None:
        from coding_agent.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from coding_agent.plan_dialog import InlinePlanWidget, PlanChoice
        from coding_agent.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from coding_agent.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from coding_agent.permission_dialog import InlinePermissionWidget
    ...
    def _update_token_label(self, input_tokens: int, output_tokens: int) -> None:
        try:
            total_in, total_out = self.get_token_count()
            # 增量事件与累计值求和（事件本身已携带累计值，这里优先用事件值）。
            if input_tokens or output_tokens:
                total_in, total_out = input_tokens, output_tokens
            in_str = _format_tokens(total_in)
            out_str = _format_tokens(total_out)
            self.query_one("#token-label", Static).update(
                f"[dim]In:[/dim] {in_str}  [dim]Out:[/dim] {out_str}"
            )
        except Exception:
            pass
