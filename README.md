...
- **Worktree 文件级隔离**：子 Agent 运行在独立 Git Worktree 中，实现文件级隔离；直接解析 `.git` 文件系统免起子进程，快速恢复延迟约 100ms → 1ms；symlink 共享大依赖避免重复复制，三重守卫自动清理陈旧工作树。通过对复杂任务做依赖解耦，拆分为互不冲突的子任务并行处理，突破单模型上下文限制，大型重构任务整体效率提升约 60%。
...
- **工具网关**（`coding_agent/tool_gateway.py`）：统一封装工具执行的稳定性——**输入防护**（限制单文件大小、超大内容分片截断防 OOM）、**异常处理**（超时兜底、指数退避重试瞬时错误、永久错误不重试）、**输出归一化**（统一截断超大输出）。
