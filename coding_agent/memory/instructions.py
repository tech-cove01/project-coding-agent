...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.coding_agent/CODING_AGENT.md, ~/.coding_agent/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 CODING_AGENT.md 和 AGENTS.md
    3. workDir/.coding_agent/INSTRUCTIONS.md（遗留格式）
    4. workDir/CODING_AGENT.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".coding_agent" / "CODING_AGENT.md")
    _add(home / ".coding_agent" / "AGENTS.md")

    # 2. 项目目录链
    for d in _project_instruction_dirs(root):
        _add(d / "CODING_AGENT.md")
        _add(d / "AGENTS.md")

    # 3. 遗留格式
    _add(root / ".coding_agent" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "CODING_AGENT.local.md")
